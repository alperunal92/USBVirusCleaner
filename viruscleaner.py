...
def carefullyFind(dirpath):
	...
	if dirname+".exe" in os.listdir(dirpath):			# if the same named exe exists,
		...
		if os.stat(path)[6]==66023L:				
			...
			print dirpath+"/"+dirname+".exe"	# the same name but not the same size, suspicious.
...
def main():
	...
	print "Welcome, this program intends to find the disk virus which copies itself in every directory with that directory's name. Provide your disk's path for the program to scan."
	print
